...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func goToAnalytics(_ sender: UIButton) {
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        if let analyticsVC = storyboard.instantiateViewController(withIdentifier: "AnalyticsViewController") as? Analytics {
            if !transactions.isEmpty {
                analyticsVC.expenseCategories = transactions.map { $0.category }
                analyticsVC.expenseValues = calculateExpenseValuesByCategory(expenseCategories: analyticsVC.expenseCategories)
    ...
    func calculateExpenseValuesByCategory(expenseCategories: [String]) -> [Double] {
        var expenseValuesByCategory: [String: Double] = [:]
        for transaction in transactions {
            if let existingAmount = expenseValuesByCategory[transaction.category] {
                expenseValuesByCategory[transaction.category] = existingAmount + (transaction.isExpense ? -transaction.amount : transaction.amount)
            } else {
                expenseValuesByCategory[transaction.category] = (transaction.isExpense ? -transaction.amount : transaction.amount)
            }
        }
        return expenseCategories.map { expenseValuesByCategory[$0] ?? 0 }
    }
